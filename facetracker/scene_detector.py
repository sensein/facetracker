...
class SceneDetector:
    ...
    def load_shots(self, input_file):
        with open(input_file, "r") as f:
            self.shots = [
                tuple(map(float, line.strip().split(","))) for line in f.readlines()[1:]
            ]
    ...
    def print_shots(self):
        ...
        for i, (start_frame, end_frame, start_time, end_time) in enumerate(self.shots):
            print(
                f"Shot {i + 1}: Start Frame = {start_frame}, End Frame = {end_frame}, Start Time = {start_time:.2f}s, End Time = {end_time:.2f}s"
            )
